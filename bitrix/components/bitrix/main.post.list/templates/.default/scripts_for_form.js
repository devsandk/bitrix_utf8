;(function(window){
	...
	window.FCForm = function(arParams)
	{
		...
		this.editor = window["BXHtmlEditor"].Get(arParams['editorId']);
		...
		this.windowEvents = {
			...
			OnUCUserQuote : BX.delegate(function(entityId, author, res, safeEdit)
			{
				if (this.entitiesId[entityId])
				{
					...
					if (!this.editor || !this.editor.toolbar.controls.Quote)
					{
						BX.DoNothing();
					}
					else if (!author && !res)
		...
		if (!this.editor)
		{
			this.windowEvents.OnEditorInitedAfter = BX.delegate(function(pEditor) { if (pEditor.id == this.editorId) { this.editor = pEditor; } }, this);
			BX.addCustomEvent(window, 'OnEditorInitedAfter', this.windowEvents.OnEditorInitedAfter);
		}
		...
		if (BX("bx-b-quote-" + this.form.id))
		{
			BX.unbindAll(BX("bx-b-quote-" + this.form.id));
			BX.bind(BX("bx-b-quote-" + this.form.id), "click", BX.delegate(function(e){
				if (this.id)
				{
					BX.PreventDefault(e);
					var quoteData = FCList.getQuoteData() || {};
					BX.onCustomEvent(window, "OnUCUserQuote", [this.id[0], quoteData['author'], quoteData['text'], false]);
				}
			}, this));
		}
		this.id = null;
	...
	window.FCForm.prototype = {
		...
		_checkTextSafety : function(id, checkObj)
		{
			...
			{
				checkObj = id;
				if (this.id && this.id.join('-') != id.join('-') && this.editor && this.editor.IsContentChanged())
		...
		show : function(id, text, data)
		{
			...
			if (this.editor)
				this.editor.CheckAndReInit(text || '');
