;(function(window){
	...
	BX.UploaderTemplateThumbnails.prototype = {
		init : function()
		{
			...
			BX.addCustomEvent(this.uploader, "onFileIsInited", this._onFileIsInited);
			...
			else
			{
				...
				if (BX(id + 'Turn'))
				{
					BX.bind(BX(id + 'Turn'), "click", BX.delegate(function() {
						if (!this.canvasTurn)
						{
							this.canvasTurn = new BX.Canvas();
							this.canvasTurn.cnv = file.canvas;
							this.canvasTurn.ctx = file.canvas.getContext('2d');
							BX.addCustomEvent(this.canvasTurn, "onChangeSize", BX.delegate(function(){
								file.canvas.style.width = "";
								file.canvas.style.height = "";
							}, file))
						}
						this.canvasTurn.rotate(true);
					}, file));
				}
			}
