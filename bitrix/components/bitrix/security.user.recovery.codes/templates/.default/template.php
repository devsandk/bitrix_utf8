	...
	<div class="code-p-wrap" id="recovery-codes-container">
		...
		<div class="code-p-block">
			...
			<div class="code-p-tip">
				<?=GetMessage("SEC_NOTICE")?>
			</div>
		</div>
		<a href="<?=$APPLICATION->GetCurPageParam('action=print')?>" class="webform-button webform-button-blue" target="_blank"><?=toUpper(GetMessage("SEC_PRINT"))?></a>
		<a href="<?=$APPLICATION->GetCurPageParam('action=download')?>" class="webform-button"><?=GetMessage("SEC_SAVE")?></a>
	</div>
	<?
	$jsCodes = array();
